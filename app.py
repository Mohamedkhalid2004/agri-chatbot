...
def calculate_water_requirement(soil, crop, weather):
    ...
    return round(base_water * crop_adjustment * weather_adjustment * 10, 2)
...
if st.button("🚀 Get Recommendations"):
    with st.spinner("⏳ Analyzing data... Please wait..."):
        time.sleep(2)
        water_needed = calculate_water_requirement(soil, crop, weather)
        harvest_date = calculate_harvest_date(crop, planting_date.strftime("%Y-%m-%d"))
        gpt_prompt = (f"I am growing {crop} in {soil} soil. The weather is {weather}. "
